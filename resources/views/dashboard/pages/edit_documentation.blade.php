			...
			<div class="page-header page-header-light shadow">
				...
				<div class="content">
					...
					<div class="card">
						...
						<div class="card-body">
                            ...
                            <form method="POST" action="{{ Route('ajout_documentations.update', $documentation) }}" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')  <!-- Ajoutez cette ligne pour indiquer une mise à jour -->
                                <div class="mb-3">
                                    <label class="form-label">Nom du document :</label>
                                    <input type="text" class="form-control" placeholder="Plaquette FIMEC" name="nom_document" value="{{ old('nom_document', $documentation->nom_document) }}">
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Année de parution :</label>
                                    <input type="text" class="form-control" placeholder="2025" value="{{ old('annee_parution', $documentation->annee_parution) }}" name="annee_parution">
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Image de couverture actuelle :</label>
                                    @if($documentation->image_couverture)
                                        <div class="mb-2">
                                            <img src="{{ asset('storage/' . $documentation->image_couverture) }}" alt="Image actuelle" class="img-thumbnail" style="max-width: 200px;">
                                        </div>
                                    @else
                                        <p class="text-muted">Aucune image de couverture.</p>
                                    @endif
                                    <label class="form-label">Nouvelle image de couverture (optionnel) :</label>
                                    <input type="file" class="form-control" name="image_couverture">
                                    <div class="form-text text-muted">Formats acceptés : jpeg, png, jpg, gif. Taille max : 2 Mo. Laissez vide pour conserver l'image actuelle.</div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    @if($documentation->fichier)
                                        <div class="mb-2">
                                            <a href="{{ asset('storage/' . $documentation->fichier) }}" target="_blank" class="btn btn-outline-secondary btn-sm">
                                                <i class="ph-download"></i> Télécharger le fichier actuel
                                            </a>
                                        </div>
                                    @else
                                    ...
                                    <div class="form-text text-muted">Formats acceptés : pdf, doc, docx, txt. Taille max : 50 Mo. Laissez vide pour conserver le fichier actuel.</div>
                                </div>

                                <div class="text-end">
                                    <button type="submit" class="btn btn-primary">Mettre à jour <i class="ph-paper-plane-tilt ms-2"></i></button>
                                </div>
                            </form>
				...
				<div class="navbar navbar-sm navbar-footer border-top">
					<div class="container-fluid">
						<span>&copy; 2026 <a href="https://themeforest.net/item/limitless-responsive-web-application-kit/13080328">Limitless Web App Kit</a></span>
