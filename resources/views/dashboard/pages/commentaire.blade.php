					...
					<div class="card">
						...
						<table class="table table-striped text-nowrap table-customers">
							<thead>
							 ...
							 @php
								$commentaires = $commentaires ?? collect();
							@endphp
							<tbody>
								...
								<tr>
									<td>
										<div class="d-flex align-items-center">
											...
											<div class="flex-fill">
												<a href="#" class="fw-semibold">{{ $commentaire->nom }}</a>
									...
									<td class="text-break" style="max-width:100px; white-space:normal;">{!! nl2br(e($commentaire->message)) !!}</td>
									<td><a href="#">{{ $commentaire->actualite->titre }}</a></td>
									<td>
										<div>
									...
									<td class="text-end">
										<div class="dropdown">
											...
											<div class="dropdown-menu dropdown-menu-end">
												<a href="#" class="dropdown-item">
													{{-- Bouton Approuver (affiché seulement si pas encore approuvé) --}}
                                                    @if(!$commentaire->is_approved)
                                                            <form action="{{ route('commentaires.approve', $commentaire) }}" method="POST">
                                                                @csrf @method('PATCH')
                                                                <button type="submit" class="btn btn-sm btn-success">✅ Approuver</button>
                                                            </form>
                                                    @endif
                                                    @if ($commentaire->is_approved)
														<form action="{{ route('commentaires.unapprove', $commentaire) }}" method="POST">
															@csrf @method('PATCH')
															<button type="submit" class="btn btn-sm btn-warning">↩️ Déapprouver</button>
														</form>
                                                    @endif
												</a>
												<a href="#" class="dropdown-item">
												...
												<a href="#" class="dropdown-item">
													{{-- Bouton Supprimer --}}
                                                    <form action="{{ route('commentaires.destroy', $commentaire) }}" method="POST" onsubmit="return confirm('Supprimer ce commentaire ?')">
                                                        @csrf @method('DELETE')
