...
@section('title', 'Liste de la presse')
						...
						<table class="table datatable-excel-background">
							<thead>
								<tr>
									...
									<th>Nom du médias</th>
                                    <th>Type de médias</th>
                                    <th>Site du médias</th>
									<th>Actions</th>
								</tr>
							</thead>
                             @php
                                ...
                                @if ($visiteurs->count()>0)
                                    @foreach ($visiteurs as $visiteur)
                                        <tr>
                                            ...
                                            <td>{{ $visiteur->nom_media }}</td>
                                            <td>{{ $visiteur->type_media }}</td>
                                            <td>{{ $visiteur->site_media }}</td>
                                            <td class="text-end">
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        <a href="#" class="dropdown-item">
                                                            <form action="{{ route('list_presses.destroy', $visiteur) }}" method="POST" onsubmit="return confirm('Confirmer la suppression ?');">
                                                            @csrf
                                                            @method('DELETE')
